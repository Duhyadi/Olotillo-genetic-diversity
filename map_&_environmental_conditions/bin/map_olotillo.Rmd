...
# Previous modification: Wed 08/Oct/2025. Versoix, Switzerland (15:00 PM) by Duhyadi Oliva-García.
...
library(sf)
```
Note #1: gpclib and maptools are no longer used; most of their functions have been migrated to other, more modern packages — primarily sf, sp, and rgeos (although rgeos is also in the process of being retired).
...
#mexico_df <- broom::tidy(mexico, region = "ENTIDAD") # broom, convert statistical objects into Tidy Tibbles
#lapply(mexico_df, class)
```
está intentando convertir el objeto sf (mexico) en un data.frame “aplanado”.
Sin embargo:

broom::tidy() ya no soporta objetos sf — fue diseñado para modelos estadísticos (lm, glm, etc.), no para datos espaciales.

```{r}
#---- Extraer coordenadas y atributos en formato data.frame ---------------------------
# Extraer coordenadas y unirlas correctamente con los atributos
coords <- st_coordinates(mexico) %>% as.data.frame()
coords$ID <- as.numeric(rownames(coords))  # crear ID temporal
# Convertir geometrías a data.frame (manteniendo atributos)
mexico_df <- mexico %>%
  st_drop_geometry() %>%              # eliminar geometría (mantiene atributos)
  mutate(ID = row_number()) %>%       # crear ID de referencia
  right_join(coords, by = "ID")       # unir con coordenadas
